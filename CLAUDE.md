...
> Sitemap: `@astrojs/sitemap` generates `sitemap-index.xml` + `sitemap-0.xml` at build (404
> filtered out). `robots.txt` is generated by `src/pages/robots.txt.js` and points at
> `sitemap-index.xml`. There is no longer a hand-maintained `public/sitemap.xml` — don't
> reintroduce one.

> SEO: `Layout.astro` is prop-driven (title, description, canonical, OG/Twitter, `noindex`).
> It emits sitewide Organization + WebSite JSON-LD; pages add their own via the `structuredData`
> prop (e.g. `FAQPage` on pricing/contact from `src/data/*Faq.js`, `SoftwareApplication` on home).
> FAQ copy lives in `src/data/pricingFaq.js` and `src/data/contactFaq.js` so the rendered
> accordion and the JSON-LD never drift — update those, not the components.
